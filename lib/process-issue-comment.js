    ...
    if (contributorUpdateQueryRes.querySucceeded) {
      ...
      if (giftCardCode) {
        ...
        if (contributorEmail) {
          ...
          const emailSucceeded = await mailer.sendGiftCardToContributor(contributorEmail, giftCardCode, contributorUpdateQueryRes.contributorLevel)
          if (emailSucceeded) {
            posthog.capture({
            ...
            await mailer.sendAlertEmail(`Unable to send email to username ${who} with email ${contributorEmail}`)
          }
        }

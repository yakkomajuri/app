...
async function processContribution({ who, contributions, log, context, commentReply = '', db, pullRequestUrl }) {
  ...
  const {
    ...
    sendMerchSucceeded,
    errorMessage
  } = await sendContributorMerch(who, isCodeContribution, db)
...
async function sendContributorMerch(who, isCodeContribution, db) {
  ...
  console.log('157', defaultResponse, {})
  ...
  console.log('167', defaultResponse, {})
  ...
  if (!contributorUpdateQueryRes.querySucceeded) {
    ...
    console.log('173', defaultResponse, res)
    return res
  }

  const giftCardCode = await db.getGiftCardCode(who, contributorUpdateQueryRes.contributorLevel, mailer)

  // No gift card available or query failed
  if (!giftCardCode) {
    let res = {...defaultResponse}
    res.errorMessage = 'Could not get a gift card code from the database.'
    console.log('183', defaultResponse, res)
    return res
  }
  ...
  if (!contributorEmail) {
    ...
    console.log('193', defaultResponse, res)
    return res
  }
  ...
  if (!emailSucceeded) {
    ...
    console.log('213', defaultResponse, res)
    return res
  }

  console.log('217', contributorEmail)
  ...
  console.log('232', defaultResponse, res)

...
const { addContributorWithDetails } = require('@posthog/all-contributors-cli')
...
class Config {
    ...
    async fetch() {
        const { content: rawOptionsFileContent, sha } = await this.repository.getFile(ALL_CONTRIBUTORS_RC)
        this.fileSha = sha
    ...
    async addContributor({ login, contributions, name, avatar_url, profile }) {
        ...
        const profileWithProtocol = profile.startsWith('http') ? profile : `http://${profile}`

        const oldContributions = this._findOldContributions(login)
        const newContributions = [...new Set([...oldContributions, ...contributions])]

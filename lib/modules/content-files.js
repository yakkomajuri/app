const { generate: generateContentFile, initBadge, initContributorsList } = require('@posthog/all-contributors-cli')
...
class ContentFiles {
    ...
    async fetch(config) {
        ...
        if (options.files.length > 15) {
            throw new AllContributorBotError(`Your .all-contributorsrc cannot contain more than 15 files.`)
        }
        this.contentFilesByPath = await this.repository.getMultipleFiles(options.files)
    }
...
function modifyFiles({ contentFilesByPath, fileContentModifierFunction }) {
    ...
    Object.entries(contentFilesByPath).forEach(([filePath, { content, sha, originalSha }]) => {

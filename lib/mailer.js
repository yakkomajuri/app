...
class Mailer {
    ...
    async sendGiftCardToContributor(contributorEmail, giftCardCode, contributorLevel) {
        ...
        await this.mg.messages().send(data, function (error) {
            if (error) {
                return false
            }
            return true
    ...
    async sendAlertEmail(message) {
        ...
        this.mg.messages().send(data, function (_, body) {
            return body && body.message && body.message.toLowerCase().includes('queued')
        })
    }
}

...
async function addContributor({
  ...
  if (isCodeContribution) {
    if (contributorEmail) {
      ...
      merchNotification = `@${who} we tried to send you an email with a merch code but that failed :( You should shame @yakkomajuri for his useless creation.
  ...
  if (config.isContributorWithRequestedContributionTypeAlreadyExists({ login: who, contributions })) {
    if (commentReply) {
      let message = merchNotification ? merchNotification : `@${who} already contributed before to ${contributions.join(", ")}`

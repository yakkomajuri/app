...
// TODO: Link to PR instead of issue if this comes from merged PR
async function addContributor({
  ...
  const requestOriginExplanation = 
    commentReply ? `This was requested by ${commentReply.replyingToWho()} [in this comment](${commentReply.replyingToWhere()})` :
    `This is an automatic action triggered by the merging of [this PR](${pullRequestURL}).`

  let merchNotification = ''

  if (merchNotificationDetails) {
    if (merchNotificationDetails.success) {
      const emailComponents = merchNotificationDetails.email.split('@')
      ...
      `@${who} we tried to send you an email with a merch code but that failed :( You should shame @yakkomajuri for his useless creation.
